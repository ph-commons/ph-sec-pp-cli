...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ph-sec-pp-cli doctor
  ph-sec-pp-cli doctor --json
  ph-sec-pp-cli doctor --fail-on warn
  ph-sec-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Free SEC Number still needs OAuth: export PH_SEC_TOKEN=\"<bearer>\" or ph-sec-pp-cli auth set-token <token> (Marketplace Free package)"
				} else {
					authConfigured = true
			...
			if os.Getenv("PH_SEC_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "PH_SEC_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PH_SEC_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/client_sec_no_status.php"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ph-sec-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("ph-sec-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'ph-sec-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'ph-sec-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'ph-sec-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'ph-sec-pp-cli sync' to refresh."
	}
	return report
}
