...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PH_SEC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PH_SEC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/ph-commons/ph-sec-pp-cli/feedback")
...
When ` + "`PH_SEC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PH_SEC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ph-sec-pp-cli feedback list
  ph-sec-pp-cli feedback list --limit 5
  ph-sec-pp-cli feedback list --json`,
