...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"field": {
			{Canonical: "sec_no", Values: []string{"sec number", "sec no", "registration number"}},
			{Canonical: "status", Values: []string{"registration status", "company status"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: ph-sec-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: ph-sec-pp-cli: learn init: %v\n", err)
		}
	})
}
