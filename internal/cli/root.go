...
type rootFlags struct {
	...
	// invocation. Mirrors the PH_SEC_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ph-sec-pp-cli",
		Short: `Free SEC Number status by registration number — local cache, honest limits. Not name search. Not GIS. Requires PH_SEC_TOKEN.`,
		Long: `Look up Philippine company registration status by SEC number (free Marketplace product only). Requires PH_SEC_TOKEN. No name search, no GIS/AFS.

Highlights (not in the official API docs):
  • company show   Show registration status for a known SEC number from the local cache, with optional live refresh.
  • which   State clearly what this free-only CLI does and does not do (no name search, no GIS/AFS).
  • search   Full-text search previously synced company status rows without network calls.
  • sql   Run SQL against the local company status store.
  • doctor   Check PH_SEC_TOKEN presence and remind free-product scope and quota.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'ph-sec-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ph-sec-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ph-sec-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelCompanyCmd(flags))
