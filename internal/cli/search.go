...
Run sync first to populate the local search index.`,
  ...
  ph-sec-pp-cli search "status"

  # Force local search explicitly
  ph-sec-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  ph-sec-pp-cli search "status" --type companies --data-source local
  # JSON output for piping
  ph-sec-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'ph-sec-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
