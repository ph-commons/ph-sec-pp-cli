...
Resource scoping:
  ...
  ph-sec-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ph-sec-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ph-sec-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ph-sec-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ph-sec-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "ph-sec-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"companies": "/client_sec_no_status.php",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "companies\x00/client_sec_no_status.php":
		return []string{"data"}
	}
	return nil
}
