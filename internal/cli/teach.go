...
// ph-sec-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "PH_SEC_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("ph-sec-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  ph-sec-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  ph-sec-pp-cli recall "<question>" --agent
  ph-sec-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ph-sec-pp-cli learnings list --agent
  ph-sec-pp-cli learnings list --query "<substring>"
  ph-sec-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  ph-sec-pp-cli learnings forget "<question>" --resource <id>
  ph-sec-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  ph-sec-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  ph-sec-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
