...
type Config struct {
	...
	PhSecToken       string          `toml:"sec_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://gwwso2.sec.gov.ph/secnumber/1.0.0",
	}
	...
	// Env var overrides
	if v := os.Getenv("PH_SEC_TOKEN"); v != "" {
		cfg.PhSecToken = v
		cfg.markEnvOverride("PhSecToken")
		cfg.AuthSource = "env:PH_SEC_TOKEN"
		cfg.CredentialSource = "env:PH_SEC_TOKEN"
	}
	...
	if v := os.Getenv("PH_SEC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PH_SEC_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "ph-sec-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.PhSecToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PH_SEC_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"sec_token":    c.PhSecToken,
			"PH_SEC_TOKEN": c.PhSecToken,
			"token":        c.PhSecToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.PhSecToken = ""
	delete(c.envOverrides, "PhSecToken")
	c.updateFileConfigField("PhSecToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PhSecToken"] {
			out.PhSecToken = c.fileConfig.PhSecToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PhSecToken":
		c.fileConfig.PhSecToken = c.PhSecToken
	}
}
