...
func withJournalHome(t *testing.T) string {
	...
	t.Setenv("PH_SEC_NO_LEARN", "")
	t.Setenv("PH_SEC_LEARN_NO_CAPTURE", "")
	t.Setenv("PH_SEC_LEARN_SESSION", "")
	return dir
}
...
func runCLI(t *testing.T, args ...string) error {
	t.Helper()
	oldArgs := os.Args
	os.Args = append([]string{"ph-sec-pp-cli"}, args...)
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("PH_SEC_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "master no-learn env", env: map[string]string{"PH_SEC_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"PH_SEC_LEARN_NO_CAPTURE": "1"}},
